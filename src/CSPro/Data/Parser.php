...
class Parser
{
	...
	* @param int $recordOcc One based occurrence of record for repeating records
	* @param int $itemOcc One based occurrence of item for repeating items
	...
	*/
	...
	{
		[$item, $record] = $this->dict->findItem($itemName);
		if ($item == false)
	...
	* in the case data.
	*
	* @param string $recordName of record in dictionary
	* @return int Number of occurrences of record
	*/
	public function getRecordOccurrences($recordName) : int
	{
	...
	* @return array Raw data for each record that matches type
	*/
	private function getAllRecordData($record) : array
	{
		...
		if (strlen($recTypeValue) == 0) {
			...
			return array_filter($this->data, fn($r) => $recTypeValue == $this->extractRecordType($r));
		}
	}
