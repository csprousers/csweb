...
use App\CSPro\Data\MapDataRepository;
...
class ReportController extends AbstractController implements TokenAuthenticatedController {
    ...
    public function showCaseFromReport(Request $request): Response {
       $this->denyAccessUnlessGranted(ReportsVoter::REPORTS_READ);
        $result = [];
        $dictionaryName = $request->get('dictionary');
        $caseId = $request->get('caseguid');
        $this->denyAccessUnlessGranted(DictionaryVoter::DATA_READ, $dictionaryName);
        if ($caseId == null) {
            $ids = $request->get('ids');

            $this->logger->debug("Getting case GUID from the data for dictionary:  $dictionaryName");
            $caseId = $this->mapDataRepository->getCaseUuids($dictionaryName, $ids)[0]["case-id"];
        }
    ...
    public function getCaseUuidsForId(Request $request): Response {
        $this->denyAccessUnlessGranted(ReportsVoter::REPORTS_READ);
        $dictionaryName = $request->get('dictionary');
        $ids = $request->get('ids');
        $this->denyAccessUnlessGranted(DictionaryVoter::DATA_READ, $dictionaryName);
        $this->logger->debug("Getting case GUIDs for all cases from the data for dictionary:  $dictionaryName");
        $caseUuids = $this->mapDataRepository->getCaseUuids($dictionaryName, $ids);

        $this->logger->debug(json_encode($caseUuids, JSON_THROW_ON_ERROR));
        $response = new Response(json_encode($caseUuids, JSON_THROW_ON_ERROR), Response::HTTP_OK);
        $response->headers->set('Content-Length', strlen($response->getContent()));
